...
UART_HandleTypeDef *da_uart_p_other=&h${USARTDmaIsTrue?string('yes', 'no')};
                   [#elseif !UsartIpInstanceList?contains(bsp.solution)]
                        [#assign UsartIpInstance = bsp.solution]
                        [#assign UsartIpInstanceList = UsartIpInstanceList + "," + bsp.solution]
                        [#assign USARTDmaIsTrue = bsp.dmaUsed]
...
UART_HandleTypeDef *da_uart_p_other=&h${USARTDmaIsTrue?string('yes', 'no')};
            ...
            [#if bsp.dmaUsed == true]
#define wereUsingDma
            [#elseif bsp.dmaUsed == true]
#define wereNotUsingDma
            [/#if]
            [#if USARTDmaIsTrue == true]
#define wereUsingDma
            [#elseif USARTDmaIsTrue == true]
#define wereNotUsingDma
            [/#if]
        [/#list]
    [/#if]
