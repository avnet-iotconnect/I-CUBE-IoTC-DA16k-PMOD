...
int uart_init(uint32_t baud, uint32_t bits, uint32_t parity, uint32_t stopbits) {
	HAL_UART_Receive_IT(&h${UsartIpInstance?lower_case}, (uint8_t *) &rxByte, 1);
	return 0;
}
...
{
	if(huart == &h${UsartIpInstance?lower_case}) {
		txDmaInUse = false;
	}
}
...
size_t uart_send(const char *src, size_t length) {
	...
	do {
		s = HAL_UART_Transmit_DMA(&h${UsartIpInstance?lower_case}, (uint8_t*)txBuf[bufSelect], length);
	}while(s != HAL_OK);
